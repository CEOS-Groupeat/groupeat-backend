...
public record OwnerProfileUpdateRequest(
        @Schema(description = "변경할 이메일. 빈 문자열 전송 시 null로 저장", example = "owner@example.com", nullable = true)
        @Email(message = "이메일 형식이 올바르지 않습니다.")
        String email,

        @Schema(description = "변경할 생년월일. 미입력 시 null로 저장", example = "1988-03-15", nullable = true)
